...
For grok crewmate and scout launches it also pre-registers folder trust in `$GROK_HOME/trusted_folders.toml` through `bin/fm-grok-trust.sh`, which needs `python3` with `tomllib` and refuses the launch rather than leaving a worker wedged on grok's unanswerable directory-security dialog.
Grok keys that grant on the repository's main worktree root, so it covers the whole project rather than the task worktree, it persists until the captain removes it, and teardown does not.
A grok secondmate is not pre-registered and can still meet that dialog; [`docs/verification/grok-folder-trust.md`](verification/grok-folder-trust.md) owns the evidence.
...
GROK_HOME=              # optional Grok config home for firstmate's global grok turn-end hook and its crew folder-trust registration; defaults to ~/.grok
