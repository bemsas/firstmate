...
- Orca's worktree shape is unverified against the spawn-time trust pre-registration helpers (`bin/fm-claude-trust.sh`, `bin/fm-agy-trust.sh`, `bin/fm-grok-trust.sh`), each of which refuses any path that is not a linked git worktree sharing the project's git common dir, so such a spawn on Orca fails loudly at that check rather than launching if Orca clones instead of linking.
