...
# classifier finds no container at all and reports `no-composer` - NOT a safe
# (empty) injection target. Covers the same guarantee herdr/cmux/tmux tests pin
# for their backends.
test_composer_state_bare_shell_prompt_is_never_empty() {
  local out
  orca_case composer-bare-shell
  printf '{"ok":true,"result":{"terminal":{"tail":["some earlier output","kunchen@mac firstmate $ "]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_composer_state term-123' "$ROOT" )
  [ "$out" = no-composer ] || fail "a bare dead-shell prompt (no bordered composer row) must read no-composer, got '$out'"
  case "$out" in
    empty|pending) fail "a dead-shell prompt must never read as an injectable composer, got '$out'" ;;
  esac
  pass "fm_backend_orca_composer_state: a bare dead-shell prompt is never an injectable empty composer"
}
