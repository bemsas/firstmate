...
# onward, so this test fails loudly if the shared isolation predicate's
# physical project comparison (bin/fm-worktree-isolation-lib.sh) ever regresses.
...
run_spawn_symlink_case() {  # <label> <physical|logical>
  ...
  # pwd -P rather than string concatenation - it must match exactly what the
  # shared isolation predicate resolves the project to, including any symlink
