...
A Herdr endpoint whose shell was recreated outside its recorded worktree is closed by `bin/fm-backend.sh`'s restored-endpoint reconciliation so relaunch reclaim applies; control-plane refusals are unchanged (`docs/herdr-backend.md`).
