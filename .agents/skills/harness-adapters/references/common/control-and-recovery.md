...
Grok gates every repository it has not trusted, and its dialog needs a `y` this control plane cannot send, so the spawn pre-registers the grant in Grok's own store before every crewmate or scout launch and refuses the launch when that fails; `references/harness/grok.md` owns the key model, the repository-wide reach, and the secondmate gap.
